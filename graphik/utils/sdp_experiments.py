...
def sdp_sol_to_point_matrix(graph, solution_dict, ee_goals):
    ...
    for idx, name in enumerate(graph.directed):
        if name == "p0":
            ...
            P_e[:, idx] = np.array([1.0, 0.0, 0.0])
        elif name == "y":
            P_e[:, idx] = np.array([0.0, 1.0, 0.0])
        elif name == "q0":
            P_e[:, idx] = np.array([0.0, 0.0, 1.0])
...
def sdp_rank3_constraint_violations(
    constraint_clique_dict, graph, solution_dict, end_effectors, ee_cost=False
):
            ...
            for v in clique:
                if frozenset((u, v)) in mapping:
                    u_val = (
                        ...
                        else solution_dict[u]
                    )
                    v_val = (
                        end_effectors[v]
                        if v in end_effectors and not ee_cost
                        else solution_dict[v]
                    )
                    residuals.append(
...
def run_sdp_rank3_convex_iteration_experiment(
    ...
    eig_value_sum = eig_value_sum_vs_iterations[-1]
    # Compute relaxation gap
    sdp_cost = prob.value
    primal_cost = -1.0  # Doesn't mean anything here
    gap = primal_cost - sdp_cost
    # Compute max constraint violation
    constraint_violations = sdp_rank3_constraint_violations(
        constraint_clique_dict, graph, solution_dict, end_effectors=ee_goals
    )
    ...
    T_goal = {
        ee[0]: graph.robot.get_pose(q_goal, ee[0]) for ee in graph.robot.end_effectors
    }
...
def run_sdp_rank3_revolute_experiment(
    ...
    primal_cost = sum(
        [
            np.linalg.norm(solution_dict[key] - nearest_points[key]) ** 2
            for key in nearest_points.keys()
        ]
    )
    gap = primal_cost - sdp_cost
    # Compute max constraint violation
