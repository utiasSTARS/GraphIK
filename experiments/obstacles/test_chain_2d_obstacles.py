...
def check_collison(P: np.ndarray, obstacles: tuple):
...
def random_problem_2d_chain():
    ...
    obstacles = [(np.array([0, 3]), 1), (np.array([3, 0]), 1), (np.array([-3, 0]), 1)]
    ...
    for idx in range(n_tests):
        ...
        sol_info = solver.solve(
            D_goal, F, use_limits=True, Y_init=X_init, bounds=(lb, ub)
        )
        Y = sol_info["x"]
        t_sol += [sol_info["time"]]
