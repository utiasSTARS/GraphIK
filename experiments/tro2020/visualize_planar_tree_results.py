...
if __name__ == '__main__':
    ...
    tol_ee = 1e-2
    tol_ang = 1e-2
    tols_ee_waterfall = list(np.linspace(-6, 1, 8))  #list(np.linspace(-6, 0, 9))

    for dof in dofs:
        planar_tree_file = "results/planar_tree_dof_"+str(dof)+"_bounded_" + str(use_limits) + \
                           "_tol_"+str(tol)+"_maxiter_2000_n_goals_"+str(n_goals)+"_n_init_1_zero_init_True_full_results.p"
        data = pickle.load(open(planar_tree_file, "rb"))
        ...
        if save:
            save_waterfall = "results/figs/waterfall_planar_tree_dof_{:}_bounded_{:}.pdf".format(dof, use_limits)
            save_boxplot = "results/figs/boxplot_planar_tree_dof_{:}_bounded_{:}.pdf".format(dof, use_limits)
        else:
            save_waterfall = None
            save_boxplot = None

        plot_y_label = (dof == dofs[0])
        make_legend = (dof == dofs[0])
