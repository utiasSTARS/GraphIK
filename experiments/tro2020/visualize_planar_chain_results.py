...
if __name__ == '__main__':
    ...
    for dof in dofs:
        planar_chain_file = "results/planar_chain_dof_"+str(dof)+"_bounded_" + str(use_limits) + \
                           "_tol_"+str(tol)+"_maxiter_2000_n_goals_"+str(n_goals)+"_n_init_1_zero_init_True_full_results.p"
        data = pickle.load(open(planar_chain_file, "rb"))
        ...
        if save:
            save_waterfall = "results/figs/waterfall_planar_chain_dof_{:}_bounded_{:}.pdf".format(dof, use_limits)
            save_boxplot = "results/figs/boxplot_planar_chain_dof_{:}_bounded_{:}.pdf".format(dof, use_limits)
        else:
            save_waterfall = None
            save_boxplot = None
