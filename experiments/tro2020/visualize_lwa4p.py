...
if __name__ == '__main__':
    ...
    lwa4p_file = "results/lwa4p_bounded_" + str(use_limits) + "_tol_" + str(tol) + "_maxiter_2000_n_goals_" + str(n_goals) + \
                "_n_init_1_zero_init_True_full_results.p"

    data = pickle.load(open(lwa4p_file, "rb"))
    ...
    if save:
        save_waterfall = "results/figs/waterfall_lwa4p_bounded_{:}.pdf".format(use_limits)
        save_boxplot = "results/figs/boxplot_lwa4p_bounded_{:}.pdf".format(use_limits)
    else:
        save_waterfall = None
        save_boxplot = None
