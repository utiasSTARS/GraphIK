...
if __name__ == '__main__':
    ...
    for dof in dofs:
        n_goals = 1000
        spherical_chain_file = "results/spherical_chain_dof_"+str(dof)+"_bounded_" + str(use_limits) + \
                           "_tol_"+str(tol)+"_maxiter_2000_n_goals_"+str(n_goals)+"_n_init_1_zero_init_True_full_results.p"
        data = pickle.load(open(spherical_chain_file, "rb"))
        ...
        if save:
            save_waterfall = "results/figs/waterfall_spherical_chain_dof_{:}_bounded_{:}.pdf".format(dof, use_limits)
            save_boxplot = "results/figs/boxplot_spherical_chain_dof_{:}_bounded_{:}.pdf".format(dof, use_limits)
        else:
            save_waterfall = None
            save_boxplot = None
